...
class Model_Table extends CI_Model{
    ...
    public function generateMaterialCode(){
        ...
        $lastRow = $this->db->query("select * from tbl_materials order by material_id desc limit 1");
        if($lastRow->num_rows() != 0){
            $newCode = $lastRow->row()->material_id + 1;
            $zeros = array('0', '00', '000');
            $code = 'M-' . (strlen($newCode) > count($zeros) ? $newCode : $zeros[count($zeros) - strlen($newCode)] . $newCode);
        }

        return $code;
    }

    public function generateMaterialSaleInvoice(){
        $code = "MS000001";
        
        $lastRow = $this->db->query("select * from tbl_material_sale order by SaleMaster_SlNo desc limit 1");
        if($lastRow->num_rows() != 0){
            $newCode = $lastRow->row()->SaleMaster_SlNo + 1;
            $zeros = array('0', '00', '000', '0000', '00000');
            $code = 'MS' . (strlen($newCode) > count($zeros) ? $newCode : $zeros[count($zeros) - strlen($newCode)] . $newCode);
        }
    ...
    public function getTransactionSummary($date = null){
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(sm.SaleMaster_PaidAmount), 0) from tbl_material_sale sm
                where sm.SaleMaster_branchid= " . $this->session->userdata('BRANCHid') . "
                and sm.Status = 'a'
                " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
            ) as received_material_sales,
            (
            ...
            (
                ...
                where mp.status = 'a'
                and mp.branch_id = " . $this->session->userdata('BRANCHid') . "
            ...
            /* total */
            (
                select received_sales + received_material_sales + received_customer + received_supplier + received_cash + bank_withdraw
            ) as total_in,
            (
    ...
    public function customerDue($clauses = "") {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(sm.SaleMaster_TotalSaleAmount), 0.00)
                ...
                and sm.Status = 'a') as productSales,

            (select ifnull(sum(sm.SaleMaster_TotalSaleAmount), 0.00)
                from tbl_material_sale sm 
                where sm.SalseCustomer_IDNo = c.Customer_SlNo
                and sm.Status = 'a') as materialSales,
            ...
            (select productSales + materialSales + ifnull(c.previous_due, 0.00)) as billAmount,
            ...
            from tbl_customer c
            left join tbl_brunch br on brunch_id = c.Customer_brunchid
