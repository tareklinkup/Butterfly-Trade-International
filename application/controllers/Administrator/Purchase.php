...
class Purchase extends CI_Controller
{
    ...
    function deletePreviousPurchase(){
        $data = json_decode($this->input->raw_input_stream);
       
        $productId = $data->productId;
        $purchaseId = $data->purchaseId;
 
        $getPSLNO = $this->db->query("SELECT PurchaseDetails_SlNo FROM tbl_purchasedetails WHERE PurchaseMaster_IDNo=?",$purchaseId)->result_array();
        $pSLNo = $getPSLNO[0]['PurchaseDetails_SlNo'];
        /// check new or old
            $getPd = $this->db->query("SELECT PurchaseDetails_SlNo  FROM tbl_purchasedetails WHERE Product_IDNo=? AND PurchaseDetails_SlNo=?",[$productId,$pSLNo])->result_array();
 
             if (empty($getPd)) {
                 echo 'add_to_cart';
             }else{
                 
         $stock = $this->db->query("SELECT p.Product_SlNo, (
               select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE sd.purchase_d_id = '$pSLNo' AND sd.Product_IDNo = '$productId'
               )  as sold_qty,
               (
                select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE sdr.purchase_d_id = '$pSLNo' AND sdr.SaleReturnDetailsProduct_SlNo = '$productId'
               ) as sold_qty_return,
               (
              select sum(pr.PurchaseReturnDetails_ReturnQuantity) as purchase_return_qty FROM tbl_purchasereturndetails as pr WHERE pr.purchase_d_id = '$pSLNo' AND pr.PurchaseReturnDetailsProduct_SlNo='$productId'
               ) as purchase_return_qty,
               
               (
                  select sum(pud.PurchaseDetails_TotalQuantity) as purchase_qty FROM tbl_purchasedetails as pud WHERE  pud.PurchaseDetails_SlNo = '$pSLNo' AND  pud.Product_IDNo='$productId'
                 ) as purchase_qty,
                 
                 ( 
                  select (IFNULL(sold_qty, 0)-IFNULL(sold_qty_return, 0))
                 ) as original_sold_qty,
             ...
             $current_quantity = $stock[0]['current_quantity'];
             $original_sold_qty = $stock[0]['original_sold_qty'];
 
 
             
 
             if ($original_sold_qty>0) {
                 echo $original_sold_qty;
             }else{
                $this->db->where('PurchaseMaster_IDNo', $data->purchaseId);
                $this->db->where('Product_IDNo', $data->productId);
                $query_result = $this->db->delete('tbl_purchasedetails');
                if ($query_result) {
                   echo 'deleted';
                }else{
                  echo 'nodeleted';
        ...
        try{
            ...
            foreach($data->cart as $product){
                $returnDetails = array(
                    ...
                    'purchase_d_id' => $product->PurchaseDetails_SlNo,
                );
    ...
    public function getPurchaseDetails(){
        ...
        $saleDetails = $this->db->query("
            ...
            left join tbl_supplier s on s.Supplier_SlNo = pm.Supplier_SlNo
            where pd.Status != 'd' and pd.PurchaseDetails_branchID = '$this->brunch'
    ...
    public function getMaterialPurchase(){
        ...
        $purchases = $this->db->query("
            ...
            where p.status = 'a' and p.branch_id = '$this->brunch' $idClause $supplierClause $dateClause
        ")->result();
