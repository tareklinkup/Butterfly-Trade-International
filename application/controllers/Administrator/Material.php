...
class Material extends CI_Controller{
    ...
    public function addMaterial(){
        ...
        try{
            ...
            $nameQuery = $this->db->query("select * from tbl_materials where name = ? and branch_id = ?", [$data->name, $this->branch]);
            ...
            if($codeCount != 0){
                $data->code = $this->mt->generateMaterialCode();
            }
    ...
    public function updateMaterial(){
        ...
        try{
            ...
            $nameQuery = $this->db->query("select * from tbl_materials where name = ? and material_id != ? and branch_id = ?", [$data->name, $data->material_id, $this->branch]);
            $nameCount = $nameQuery->num_rows();
            if($nameCount != 0){
                $res = ['success'=>false, 'message'=>'Duplicate material name ' . $data->name];
                echo json_encode($res);
    ...
    public function getMaterialStock(){
        ...
        $stock = $this->db->query("
            select
                ...
                ifnull((select sum(SaleDetails_TotalQuantity) from tbl_material_sale_details where material_id = m.material_id and Status = 'a'), 0.00) as sold_quantity,
                (select purchased_quantity - (production_quantity + damage_quantity + sold_quantity)) as stock_quantity
            from
            tbl_materials m
