...
class Sales extends CI_Controller {
    ...
    public function marge(){
        ...
        $pdetailQn = $this->db->query("SELECT PurchaseDetails_TotalQuantity as pqty,PurchaseDetails_SlNo ,PurchaseDetails_SlNo as sl FROM tbl_purchasedetails WHERE Product_IDNo=?",309)->result_array();
                 
                 
        $productiondetailQn = $this->db->query("SELECT quantity as prodqty,production_products_id as production_d_id,production_products_id as sl FROM tbl_production_products WHERE product_id=?",309)->result_array();
        $big_array = array_merge_recursive($pdetailQn,$productiondetailQn);
        
      
         
        
        //$oneDimensionalArray = array_map('current', $big_array);
    ...
    //     print_r($f);
        ...
        //print_r(array_merge_recursive($pdetailQn,$productiondetailQn));
       // echo '</pre>';
    ...
    public function addSales(){
        ...
        try{
            ...
            foreach($data->cart as $cartProduct){
                ...
                if($cartProduct->from=='normal'){
                   

                     // Checking purchse Lot
                $pdetailQn = $this->db->query("SELECT PurchaseDetails_SlNo,PurchaseDetails_Rate,PurchaseDetails_TotalQuantity as pqty,PurchaseDetails_SlNo FROM tbl_purchasedetails WHERE Product_IDNo=? and PurchaseDetails_branchID=? and Status='a' and PurchaseDetails_SlNo!=0 ",[$cartProduct->productId,$this->sbrunch])->result_array();
                $pQtyT = 0;
                $newPQtyT = 0;

                $current_quantity_total = 0;
                $next = $cartProduct->quantity;
                foreach ($pdetailQn as $key => $value) {
                $pQtyT = $pQtyT+$value['pqty'];
                $pqty = $value['pqty'];
                
                $productId = $cartProduct->productId;
                $lot_id = $value['PurchaseDetails_SlNo']; 

            $current_qty_get = $this->db->query("SELECT p.Product_SlNo, (
              select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE sd.purchase_d_id = '$lot_id' AND sd.Product_IDNo = '$productId' and sd.Status='a'
              )  as sold_qty,
              (
               select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE sdr.purchase_d_id = '$lot_id' AND sdr.SaleReturnDetailsProduct_SlNo = '$productId'
              ) as sold_qty_return,
              (
             select sum(pr.PurchaseReturnDetails_ReturnQuantity) as purchase_return_qty FROM tbl_purchasereturndetails as pr WHERE pr.purchase_d_id = '$lot_id' AND pr.PurchaseReturnDetailsProduct_SlNo='$productId'
              ) as purchase_return_qty,
              
              (
                 select sum(pud.PurchaseDetails_TotalQuantity) as purchase_qty FROM tbl_purchasedetails as pud WHERE  pud.PurchaseDetails_SlNo = '$lot_id' AND  pud.Product_IDNo='$productId' and pud.Status='a'
                ) as purchase_qty,

              (
              select (IFNULL(purchase_qty,0)-IFNULL(sold_qty, 0))+(IFNULL(sold_qty_return, 0))-(IFNULL(purchase_return_qty, 0))
              ) as current_quantity 
            FROM tbl_product p
          
            WHERE p.Product_SlNo='$productId'")->result_array();

            $current_quantity = $current_qty_get[0]['current_quantity'];
            
            if ($current_quantity <= 0) {
               continue;
            }
            if ($current_quantity>=$next) {
                $saleDetails = array(
                    'purchase_d_id'=>$value['PurchaseDetails_SlNo'],
                    'SaleMaster_IDNo' => $salesId,
                    'Product_IDNo' => $cartProduct->productId,
                    'SaleDetails_TotalQuantity' => $next,
                    ...
                    where product_id = ?
                    and branch_id = ?
                    ", [$next, $cartProduct->productId, $this->session->userdata('BRANCHid')]);


                    break;
            }

            if ($next>$current_quantity) {
                $setNextQty = $next-$current_quantity;
            }else{
                $setNextQty = $current_quantity-$next;
            }
            $next       = $setNextQty;
            ...
            $saleDetails = array(
                    ...
                    'SaleMaster_IDNo' => $salesId,
                    'Product_IDNo' => $cartProduct->productId,
                    'SaleDetails_TotalQuantity' => $current_quantity,
                    'Purchase_Rate' => $value['PurchaseDetails_Rate'],
                    'SaleDetails_Rate' => $cartProduct->salesRate,
                    'SaleDetails_Tax' => $cartProduct->vat,
                    'SaleDetails_TotalAmount' => $cartProduct->total,
                    ...
                    where product_id = ?
                    and branch_id = ?
                    ", [$current_quantity, $cartProduct->productId, $this->session->userdata('BRANCHid')]);
                if ($next==0) {
                    break;
                }
                ...
                if($cartProduct->from=='production'){
                // Checking purchse Lot
                $pdetailQn = $this->db->query("SELECT price,quantity as pqty,production_products_id as PurchaseDetails_SlNo FROM tbl_production_products WHERE product_id=? and branch_id=? and status='a' ",[$cartProduct->productId,$this->sbrunch])->result_array();
                ...
                $current_qty_get = $this->db->query("SELECT p.Product_SlNo, (
                    select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE  sd.production_d_Id = '$lot_id' AND  sd.Product_IDNo = '$productId' AND sd.Status='a'
                    )  as sold_qty,
                    (
                     select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE sdr.production_d_Id='$lot_id' AND sdr.SaleReturnDetailsProduct_SlNo = '$productId' AND sdr.Status='a'
                    ) as sold_qty_return,
                    (
                     select sum(pd.quantity) as production_qty FROM tbl_production_products as pd WHERE pd.production_products_id='$lot_id' AND pd.product_id = '$productId' AND pd.status='a'
                    ) as production_qty,
                    ( 
                    select (IFNULL(sold_qty_return, 0)+IFNULL(production_qty,0)) -(IFNULL(sold_qty, 0))
                    ) as current_quantity   
            ...
            if ($current_quantity>=$next) {
                $saleDetails = array(
                    'production_d_Id'=>$value['PurchaseDetails_SlNo'],
                    'SaleMaster_IDNo' => $salesId,
                    'Product_IDNo' => $cartProduct->productId,
                    'SaleDetails_TotalQuantity' => $next,
                    'Purchase_Rate' => $value['price'],
                    'SaleDetails_Rate' => $cartProduct->salesRate,
                    'SaleDetails_Tax' => $cartProduct->vat,
                    'SaleDetails_TotalAmount' => $cartProduct->total,
                    'Status' => 'a',
                    ...
                    $this->db->query("
                    update tbl_currentinventory 
                    set production_sale_qty = production_sale_qty + ? 
                    where product_id = ?
                    and branch_id = ?
                    ", [$next, $cartProduct->productId, $this->session->userdata('BRANCHid')]);
            ...
            $message = "{$sendToName},\nআজকের আপনার মোট ক্রয়ের পরিমাণ {$data->sales->total} টাকা। জমা হয়েছে {$data->sales->paid} টাকা, {$data->sales->salesDate} তারিখে। মোট বাকি আছে {$currentDue} টাকা। চালান নং. {$invoice}";
    ...
    public function updateSales(){
        ...
        try{
            ...
            foreach($currentSaleDetails as $product){
                ...
                if($product->sale_from=='production'){
                    $this->db->query("
                    update tbl_currentinventory 
                    set production_sale_qty = production_sale_qty - ? 
                    where product_id = ?
            ...
            foreach($data->cart as $cartProduct){
                    ...
                    $pdetailQn = $this->db->query("SELECT PurchaseDetails_Rate,PurchaseDetails_SlNo,PurchaseDetails_TotalQuantity as pqty,PurchaseDetails_SlNo FROM tbl_purchasedetails WHERE Product_IDNo=? and PurchaseDetails_branchID=? and  Status='a' and PurchaseDetails_SlNo!=0",[$cartProduct->productId,$this->sbrunch])->result_array();
                    $pQtyT = 0;
                    $newPQtyT = 0;
                ...
                $current_qty_get = $this->db->query("SELECT p.Product_SlNo, (
                  select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE sd.purchase_d_id = '$lot_id' AND sd.Product_IDNo = '$productId' and sd.SaleDetails_BranchId='$this->sbrunch'
                  )  as sold_qty,
                  (
                   select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE sdr.purchase_d_id = '$lot_id' AND sdr.SaleReturnDetailsProduct_SlNo = '$productId' and sdr.SaleReturnDetails_brunchID='$this->sbrunch'
                  ) as sold_qty_return,
                  (
                 select sum(pr.PurchaseReturnDetails_ReturnQuantity) as purchase_return_qty FROM tbl_purchasereturndetails as pr WHERE pr.purchase_d_id = '$lot_id' AND pr.PurchaseReturnDetailsProduct_SlNo='$productId' and pr.PurchaseReturnDetails_brachid='$this->sbrunch'
                  ) as purchase_return_qty,
                  
                  (
                     select sum(pud.PurchaseDetails_TotalQuantity) as purchase_qty FROM tbl_purchasedetails as pud WHERE  pud.PurchaseDetails_SlNo = '$lot_id' AND  pud.Product_IDNo='$productId' and pud.PurchaseDetails_branchID='$this->sbrunch'
                    ) as purchase_qty,
                ...
                $saleDetails = array(
                    ...
                    $current_qty_get = $this->db->query("SELECT p.Product_SlNo, (
                        select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE  sd.production_d_Id = '$lot_id' AND  sd.Product_IDNo = '$productId' AND sd.Status='a' and sd.SaleDetails_BranchId='$this->sbrunch'
                        )  as sold_qty,
                        (
                         select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE sdr.production_d_Id='$lot_id' AND sdr.SaleReturnDetailsProduct_SlNo = '$productId' AND sdr.Status='a' and  sdr.SaleReturnDetails_brunchID='$this->sbrunch'
                        ) as sold_qty_return,
                        (
                         select sum(pd.quantity) as production_qty FROM tbl_production_products as pd WHERE pd.production_products_id='$lot_id' AND pd.product_id = '$productId' AND pd.status='a' and pd.branch_id='$this->sbrunch'
                        ) as production_qty,
                        ( 
                        select (IFNULL(sold_qty_return, 0)+IFNULL(production_qty,0)) -(IFNULL(sold_qty, 0))
                        ) as current_quantity   
    ...
    public function getSaleDetailsForReturn(){
        ...
        $saleDetails = $this->db->query("
            ...
            p.Product_Name,
            pc.ProductCategory_Name,
            ifnull(sum(srd.SaleReturnDetails_ReturnQuantity),0) as returned_quantity,
            ifnull(sum(srd.SaleReturnDetails_ReturnAmount),0) as returned_amount
            from tbl_saledetails sd
            left join tbl_product p on p.Product_SlNo = sd.Product_IDNo
            left join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
            ...
            and sd.sale_from = srd.sale_from 
            and sd.Product_IDNo = srd.SaleReturnDetailsProduct_SlNo  
            where sd.SaleMaster_IDNo = ?
        ...
        try{
            ...
            foreach($data->cart as $product){
                $returnDetails = array(
                    ...
                    'purchase_d_id' => $product->purchase_d_id,
                    'production_d_Id' => $product->production_d_Id,
                ...
                if($product->sale_from=='production'){
                    $this->db->query("
                    update tbl_currentinventory 
                    set production_return_qty = production_return_qty + ? 
                    where product_id = ?
                    and branch_id = ?
                ", [$product->return_quantity, $product->Product_IDNo, $this->session->userdata('BRANCHid')]);
                }
        ...
        foreach($sales as $sale){
            $sale->saleDetails = $this->db->query("
                select
                    ...
                    (sd.SaleDetails_Rate * sd.SaleDetails_TotalQuantity) as sold_amount,
