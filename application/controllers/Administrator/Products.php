...
class Products extends CI_Controller {
    ...
    public function addProduct() {
        ...
        try{
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and Product_branchid = ? and status='a'", [$productObj->Product_Name, $this->brunch])->num_rows();
            if($productNameCount > 0){
                $res = ['success'=>false, 'message'=>'Product name already exists on this branch'];
                echo json_encode($res);
                exit;
            }

            $productCodeCount = $this->db->query("select * from tbl_product where Product_Code = ? and Product_branchid = ?", [$productObj->Product_Code, $this->brunch])->num_rows();
    ...
    public function updateProduct(){
        ...
        try{
            ...
            // $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and Product_branchid = ? and Product_SlNo != ?", [$productObj->Product_Name, $this->brunch, $productObj->Product_SlNo])->num_rows();
            // if($productNameCount > 0){
    ...
    public function getProductStock(){
        ...
        if($stockCount != 0){
            ...
            $stock = ($stockRow->purchase_quantity + $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity +$stockRow->production_return_qty+ $stockRow->production_quantity) 
                    - ($stockRow->sales_quantity  + $stockRow->production_sale_qty + $stockRow->purchase_return_quantity + $stockRow->damage_quantity + $stockRow->transfer_from_quantity);
        
                $purchaseStock = 0;
                $purchaseStock = ($stockRow->purchase_quantity + $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity) 
                - ($stockRow->sales_quantity + $stockRow->purchase_return_quantity + $stockRow->damage_quantity + $stockRow->transfer_from_quantity);
               
                $productionStock = ($stockRow->production_quantity+$stockRow->production_return_qty)-($stockRow->production_sale_qty);

                }

        
            $getStock =  (object)array('stock'=>$stock,'normalStock'=>$purchaseStock,'productionStock'=>$productionStock);
    ...
    public function getCurrentStock(){
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ci.*,
                    (select (ci.purchase_quantity + ci.sales_return_quantity + ci.transfer_to_quantity + ci.production_quantity+ci.production_return_qty) - (ci.sales_quantity + ci.production_sale_qty + ci.purchase_return_quantity + ci.damage_quantity + ci.transfer_from_quantity)) as current_quantity,
                    p.Product_Name,
                    p.Product_Code,
    ...
    public function getProductionLotStock(){
        $current_stock = $this->db->query("SELECT pd.*,p.Product_Code,p.Product_Name,p.Product_Purchase_Rate,pd.price,
        (
        select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE pd.production_products_id = sd.production_d_Id AND pd.product_id = sd.Product_IDNo AND sd.Status='a' and sd.SaleDetails_BranchId='$this->brunch'
        )  as sold_qty,
        (
         select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE pd.production_products_id = sdr.production_d_Id AND pd.product_id = sdr.SaleReturnDetailsProduct_SlNo AND sdr.Status='a' and  sdr.SaleReturnDetails_brunchID='$this->brunch'
        ) as sold_qty_return,
        (
        select (IFNULL(pd.quantity,0)-IFNULL(sold_qty, 0))+(IFNULL(sold_qty_return, 0))
        ) as current_quantity 
      FROM tbl_production_products pd  
      LEFT JOIN tbl_product p ON pd.product_id = p.Product_SlNo 
      where pd.status='a' and pd.branch_id = '$this->brunch'
       ORDER BY pd.production_products_id  ASC ")->result();
      echo json_encode($current_stock);
    }

    public function getRegulerLotStock(){
        $current_stock = $this->db->query("SELECT pd.*,p.Product_Code,p.Product_Name,p.Product_Purchase_Rate,pd.PurchaseDetails_Rate ,
              (
              select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE pd.PurchaseDetails_SlNo = sd.purchase_d_id AND pd.Product_IDNo = sd.Product_IDNo AND sd.Status='a' and sd.SaleDetails_BranchId='$this->brunch'
              )  as sold_qty,
              (
               select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE
               pd.PurchaseDetails_SlNo = sdr.purchase_d_id AND 
               pd.Product_IDNo = sdr.SaleReturnDetailsProduct_SlNo AND sdr.Status='a' and 
               sdr.SaleReturnDetails_brunchID='$this->brunch'
               
              ) as sold_qty_return,
              (
             select sum(pr.PurchaseReturnDetails_ReturnQuantity) as purchase_return_qty FROM tbl_purchasereturndetails as pr WHERE pd.PurchaseDetails_SlNo = pr.purchase_d_id AND pd.Product_IDNo = pr.PurchaseReturnDetailsProduct_SlNo AND Status='a' and PurchaseReturnDetails_brachid='$this->brunch'
            ...
            where pd.Status = 'a' and pd.PurchaseDetails_branchID='$this->brunch'
            and pd.PurchaseDetails_SlNo != 0
             ORDER BY pd.PurchaseDetails_SlNo  ASC ")->result();
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(pp.quantity), 0)
                        ...
                        join tbl_product prd on prd.Product_SlNo = pp.product_id 
                        where pp.product_id = p.Product_SlNo
                        and prd.Product_branchid = '$branchId') as production_quantity,
