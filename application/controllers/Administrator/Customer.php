...
class Customer extends CI_Controller
{
    ...
    public function addCustomerPayment(){
        ...
        try{
            ...
            $message = "{$customerInfo->Customer_Name}.\nআপনার বকেয়া পরিশোধের পরিমাণ  {$paymentObj->CPayment_amount} টাকা। বর্তমান বকেয়া {$currentDue} টাকা।";
    ...
    public function addCustomer()
    {
        ...
        try{
            ...
            $customerMobileCount = $this->db->query("select * from tbl_customer where Customer_Mobile = ? and Customer_brunchid = ?", [$customerObj->Customer_Mobile, $this->cbrunch])->num_rows();
    ...
    public function DueSms(){
        $data['title'] = "Send Customer Due SMS";
        $data['content'] = $this->load->view('Administrator/SMS/due_sms', $data, true);
        $this->load->view('Administrator/index', $data);
    }

    public function SendDueSms()
    {
        $res = ['success' => false, 'message' => ''];
        try {
            $data = json_decode($this->input->raw_input_stream);
            
            $customers = $data->numbers;
            $recipients = [];
            foreach($customers as $customer){
                $arr = [
                    'number' => $customer->Customer_Mobile,
                    'message' => "{$customer->Brunch_name} প্রিয়  গ্রাহক আপনার  বর্তমান বকেয়া  {$customer->dueAmount} টাকা। আপনাকে অতিসত্বর আপনার প্রদেয় পাওনা পরিশোধ করার জন্য বিশেষভাবে অনুরোধ করা হলো ।"
                ];
                array_push($recipients,$arr);
            }

            $this->sms->sendBulkSms('', '', $recipients);
            $res = ['success' => true, 'message' => 'Successfully Message Send'];
        } catch (\Exception $e) {
            $res = ['success' => false, $e->getMessage()];
        }
