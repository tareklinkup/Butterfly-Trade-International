						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									...
									<div class="col-sm-3">
										<input type="text" id="purchaseRate" name="purchaseRate" class="form-control" placeholder="Pur. Rate" v-model="selectedProduct.Product_Purchase_Rate" />
									</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td>{{ product.total }}</td>
							<td><a href="" v-on:click.prevent="removeFromCart(sl,product.productId)"><i class="fa fa-trash"></i></a></td>
						</tr>
...
<script>
	...
	new Vue({
		...
		methods:{
			...
			async addToCart(){
				let long = false;
				await axios.post('/check_original_sold_qty', {purchaseId:this.purchase.purchaseId,productId:this.selectedProduct.Product_SlNo}).then(res => {
						 if(res.data ==='add_to_cart'){
                           return true;
						}else{
							if (this.selectedProduct.quantity<res.data) {
								alert("You quantity must be long than "+res.data +" or Equal (Sold Quantity is : "+res.data +' '+") !!");
								long = true;
							}else{
								return true;
							}
				...
				let cartItem = this.cart.filter(p => p.productId == this.selectedProduct.Product_SlNo);
				// if(cartInd > -1){
		       ...
		       /// cartItem = Object.assign({}, cartItem)
				...
				// if (cartInd>0 && cartItem[0].hasOwnProperty('PurchaseDetails_SlNo')) {
					//setSl = cartItem[0].PurchaseDetails_SlNo
//				}
			...
			removeFromCart(ind,productId){
				if (this.purchase.purchaseId != 0) {
					axios.post('/delete_previous_purchase', {productId: productId,purchaseId:this.purchase.purchaseId}).then(res => {
						if (res.data ==='deleted') {
							window.location.reload();
						}else if(res.data ==='add_to_cart'){
                            this.cart.splice(ind, 1);
				            this.calculateTotal();
						}else{
							alert("Sorry this product already sold (Sold Quantity is : "+res.data +' '+") !! You can just update quantity. please add to cart this product.");
						}
				})
				}else{
				this.cart.splice(ind, 1);
				...
				axios.post('/get_purchases', {purchaseId: this.purchase.purchaseId}).then(res => {
					...
					r.purchaseDetails.forEach(product => {
						let cartProduct = {
							...
							PurchaseDetails_SlNo: product.PurchaseDetails_SlNo,
						}
