					...
					<div class="row">
						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								...
								<label class="col-sm-5 control-label no-padding-right"> From </label>
								<div class="col-sm-7" style="display:flex"> 
								    <input type="radio" name="salesType" value="normal" v-model="selectedProduct.from" v-on:change="onFromTypeChange"> Reguler 
									<input type="radio" name="salesType" value="production" v-model="selectedProduct.from" v-on:change="onFromTypeChange"> Production
								</div> 
								</div>
						...
						<div class="col-sm-2">
							<div style="display:none;" v-bind:style="{display:sales.isService == 'true' ? 'none' : ''}">
								...
								<input type="text" id="stockUnit" v-model="selectedProduct.Unit_Name" readonly style="border:none;font-size:12px;width:100%;text-align: center;"><br>
							
							

								<p style="margin:0;padding:0">Reguler Stock</p>
								<input type="text" id="normalStock" v-model="normalStock" readonly style="border:none;font-size:20px;width:100%;text-align:center;color:green"><br>


								<p  style="margin:0;padding:0">Production  Stock</p>
								<input type="text" id="productionStock" v-model="productionStock" readonly style="border:none;font-size:20px;width:100%;text-align:center;color:green"><br>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr class="">
							...
							<th style="width:20%;color:#000;">From</th>
		...
		methods:{
			...
			async productOnChange(){
				if((this.selectedProduct.Product_SlNo != '' || this.selectedProduct.Product_SlNo != 0) && this.sales.isService == 'false'){
					let stock =  await axios.post('/get_product_stock', {productId: this.selectedProduct.Product_SlNo}).then(res => {
						return res.data;
					})


					this.productStock = stock.stock;
					this.normalStock = stock.normalStock;
					this.productionStock = stock.productionStock;
			...
			addToCart(){
				let product = {
					...
					from: this.selectedProduct.from
				}
				...
				// if(this.productStock == undefined || product.quantity > this.productStock && this.sales.isService == 'false'  ){
				// 	alert('Stock unavailable');
				// 	return;
				// }
				 
				 if((this.selectedProduct.from=='normal' ) && (product.quantity > parseFloat(this.normalStock) && this.sales.isService == 'false' || this.normalStock == undefined) ){
				 	alert('Normal Stock unavailable');
				 	return;
				 }
				
				 
				 if((this.selectedProduct.from=='production' ) && (product.quantity > parseFloat(this.productionStock) || this.productionStock == undefined) ){
				 	alert('Production Stock unavailable');
				 	return;
				 }

				let cartInd = this.cart.findIndex(p => p.productId == product.productId && p.from == product.from);
				if(cartInd > -1){
