                    ...
                    <tbody>
                        <template v-for="production in productions">
                            <tr>
                                ...
                                <td style="text-align:left;">{{ production.note }}</td>
                                <td style="text-align:left;">{{ production.products[0].name }}</td>
                                <td style="text-align:right;">{{ production.products[0].quantity }}</td>
                                <td>
                                    <a href="" v-bind:href="`/production_invoice/${production.production_id}`" target="_blank"><i class="fa fa-file-text fa-2x"></i></a>
                                    <?php if($this->session->userdata('accountType') != 'u'){?>
                                    <a href="" v-bind:href="`/production/edit/${production.production_id}`"><i class="fa fa-pencil-square fa-2x"></i></a>
                                    <a href="" v-on:click.prevent="deleteProduction(production.production_id)"><i class="fa fa-trash fa-2x"></i></a>
                                    <?php }?>
                                </td>
                            </tr>
                            <tr v-for="(product, sl) in production.products.slice(1)">
                                <td colspan="6" v-bind:rowspan="production.products.length - 1" v-if="sl == 0"></td>
