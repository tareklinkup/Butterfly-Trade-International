				...
				<div class="col-xs-6">
					<table class="balancesheet-table">
						...
						<tbody>
							...
							<tr>
								<td>Total Material Sales</td>
								<td style="text-align:right;">{{ totalMaterialSales | decimal }}</td>
							</tr>
							<tr>
	...
	new Vue({
		...
		data() {
			return {
				...
				employeePayments: [],
				showReport: null,
				branchId: parseInt("<?php echo $this->session->userdata('BRANCHid'); ?>")
			}
		},
		filters: {
			decimal(value) {
		...
		computed: {
			...
			totalMaterialSales() {
				return this.materialSales.reduce((prev, curr) => {
					return prev + parseFloat(curr.SaleMaster_PaidAmount)
				}, 0).toFixed(2);
			},
			totalPurchase() {
			...
			totalCashIn() {
				return parseFloat(this.totalSales) +
					parseFloat(this.totalMaterialSales) +
		...
		methods: {
			...
			async getMaterialSales() {
				await axios.post('/get_material_sales', this.filter)
					.then(res => {
