						...
						<template v-for="sale in sales">
							<tr>
								...
								<td style="text-align:center;">
									<a href="" title="Sale Invoice" v-bind:href="`/material_sale_invoice/${sale.SaleMaster_SlNo}`" target="_blank"><i class="fa fa-file"></i></a>
									<?php if($this->session->userdata('accountType') != 'u'){?>
									<a href="" title="Edit Sale" v-bind:href="`/material_sale/${sale.SaleMaster_SlNo}`"><i class="fa fa-edit"></i></a>
									<a href="" title="Delete Sale" @click.prevent="deleteSale(sale.SaleMaster_SlNo)"><i class="fa fa-trash"></i></a>
									<?php }?>
								</td>
							</tr>
							<tr v-for="(material, sl) in sale.saleDetails.slice(1)">
								<td colspan="5" v-bind:rowspan="sale.saleDetails.length - 1" v-if="sl == 0"></td>
								<td>{{ material.name }}</td>
								<td style="text-align:right;">{{ material.SaleDetails_Rate }}</td>
								<td style="text-align:center;">{{ material.SaleDetails_TotalQuantity }}</td>
								<td style="text-align:right;">{{ material.SaleDetails_TotalAmount }}</td>
								<td></td>
							</tr>
	...
	new Vue({
		...
		methods: {
			...
			getSalesRecord(){
				...
				let url = '/get_material_sales';
				if(this.recordType == 'with_details'){
					url = '/get_material_sales_record';
				}
			...
			getSaleDetails(){
				let filter = {
					categoryId: this.selectedCategory == null || this.selectedCategory.MaterialCategory_SlNo == '' ? '' : this.selectedCategory.MaterialCategory_SlNo,
			...
			deleteSale(saleId){
				...
				axios.post('/delete_material_sale', {saleId: saleId})
				.then(res => {
					let r = res.data;
					alert(r.message);
					if(r.success){
			...
			async print(){
				...
				let categoryText = '';
				if(this.selectedCategory != null && this.selectedCategory.MaterialCategory_SlNo != '' && this.searchType == 'category'){
				...
				let reportContent = `
					<div class="container">
						...
						<div class="row">
							<div class="col-xs-6">
								${userText} ${customerText} ${employeeText} ${materialText} ${categoryText}
							</div>
