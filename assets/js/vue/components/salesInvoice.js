                        ...
                        <table _a584de>
                            ...
                            <tbody>
                                <tr v-for="(product, sl) in cart">
                                    ...
                                    <td>{{ parseFloat(product.SaleDetails_TotalQuantity).toFixed(2) }} {{ product.Unit_Name }}</td>
                                    <td>{{ product.SaleDetails_Rate }}</td>
                                    <td align="right">{{ parseFloat(product.SaleDetails_TotalAmount).toFixed(2) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    methods:{
        ...
        groupBy(array){
            ...
            array.reduce(function(res, value) {
                if (!res[value.Product_IDNo]) {
                    res[value.Product_IDNo] = { Product_IDNo: value.Product_IDNo, SaleDetails_TotalQuantity: 0,SaleDetails_TotalAmount:0,Product_Name:value.Product_Name,
                        SaleDetails_Rate:value.SaleDetails_Rate};
                    result.push(res[value.Product_IDNo])
                  }
                  res[value.Product_IDNo].SaleDetails_TotalQuantity += parseFloat(value.SaleDetails_TotalQuantity);
                  res[value.Product_IDNo].SaleDetails_TotalAmount += parseFloat(value.SaleDetails_TotalQuantity) * parseFloat(value.SaleDetails_Rate);
                  return res;
            }, {});
        ...
        getSales(){
            axios.post('/get_sales', {salesId: this.sales_id}).then(res=>{
                ...
                this.cart = this.groupBy(res.data.saleDetails)
